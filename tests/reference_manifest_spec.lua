local reference_manifest = require("reaprojectlink.reference_manifest")
local json = require("reaprojectlink.json")

local snapshot, pointer = reference_manifest.build({
  reference_id = "reference-1",
  master_project_id = "master-1",
  reference_revision = 3,
  master_project_name = "CIN_030_MASTER",
  published_at = "2026-09-13T14:00:00+08:00",
  published_by = "Alice",
  sample_rate = 48000,
  reference_start_samples = 96000,
  reference_start_marker_id = "marker-1",
  frame_rate = { numerator = 24000, denominator = 1001, drop_frame = false },
  project_timecode_offset_samples = 3600000,
  lanes = {
    {
      laneId = "lane-1",
      displayName = "Reference Main",
      order = 0,
      items = {
        {
          itemId = "item-1",
          videoFile = "//nas/show/CIN_030_v003.mov",
          videoHash = "sha256:abc123",
          startSamples = 96000,
          sourceOffsetSamples = 0,
          durationSamples = 144000,
          playbackRate = 1,
        },
      },
    },
  },
  markers = { { entryId = "marker-1", name = "FFOP", startSamples = 96000 } },
  regions = {},
})

assert(snapshot.schemaVersion == 2, "schema version")
assert(snapshot.referenceId == "reference-1", "Reference identity")
assert(snapshot.masterProjectId == "master-1", "Master Project identity")
assert(snapshot.referenceRevision == 3, "Reference revision")
assert(snapshot.lanes[1].items[1].videoHash == "sha256:abc123", "video hash")
assert(snapshot.timeline.frameRate.denominator == 1001, "frame-rate denominator")
assert(snapshot.timeline.referenceStartMarkerId == "marker-1", "Reference Start Marker")
assert(json.encode(snapshot.regions) == "[]", "empty Regions encode as an array")
assert(pointer.latestReferenceRevision == 3, "stable pointer revision")
assert(pointer.masterProjectId == "master-1", "pointer Master Project identity")
assert(pointer.manifest == "history/reference-0003.json", "history path")

return 1

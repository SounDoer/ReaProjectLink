...
local function memory_filesystem(initial, options)
  ...
  function fs.write_file(path, bytes)
    if options.corrupt_snapshot and path:match("delivery%-%d+%.json$") then
      files[path] = "{broken"
    else
      files[path] = bytes
    end
...
local result, err = delivery_writer.publish({
...
assert(
  json.decode(fs.read_file("package/history/delivery-0001.json")).deliveryRevision == 1,
  "immutable snapshot must be visible"
)
assert(
  json.decode(fs.read_file("package/delivery.json")).latestDeliveryRevision == 1,
...
local failed = delivery_writer.publish({
...
local interrupted = delivery_writer.publish(retry_input, retry_fs)
...
local retried, retry_error = delivery_writer.publish(fresh_retry, retry_fs)
assert(retried, retry_error)
assert(json.decode(retry_fs.read_file("retry-package/delivery.json")).latestDeliveryRevision == 1, "retry commits pointer")

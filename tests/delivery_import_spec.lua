...
local pointer_path = "C:/source/_ReaProjectLink/DX/delivery.json"
local snapshot_path = "C:/source/_ReaProjectLink/DX/history/delivery-0002.json"
local media_path = "C:/source/_ReaProjectLink/DX/media/clip-1/Line_r0002.wav"
local pointer = {
  schemaVersion = 1,
...
local snapshot = {
  ...
  deliveryRevision = 2,
  reference = { referenceId = "reference-1", reviewedRevision = 6 },
  sampleRate = 48000,
  lanes = {
    {
      laneId = "lane-1",
...
function adapter.create_master_track(name)
...
local review, review_error = delivery_import.review(adapter, fs, pointer_path)
assert(review, review_error)
assert(review.blocker_count == 0, "valid delivery is importable")
assert(review.reference_warning, "older reviewed Reference is warned")
...
local escaped, escaped_error = delivery_import.review(adapter, fs, pointer_path)
assert(
...
files[snapshot_path] = json.encode(snapshot)
local duplicated, duplicated_error = delivery_import.review(adapter, fs, pointer_path)
assert(
...
files[snapshot_path] = json.encode(snapshot)
local malformed, malformed_error = delivery_import.review(adapter, fs, pointer_path)
assert(
...
local stale_reference, stale_reference_error = delivery_import.apply(review, adapter, {
  allow_reference_revision_mismatch = true,
  mappings = {
    ["lane-1"] = { kind = "create" },
    ["lane-2"] = { kind = "unmapped" },
  },
})
assert(
  not stale_reference and stale_reference_error:find("Reference state changed", 1, true),
  "a changed Reference anchor makes Import Review stale"
)
values.reference_start_samples = "96000"

local result, apply_error = delivery_import.apply(review, adapter, {
  allow_reference_revision_mismatch = true,
  mappings = {
    ["lane-1"] = { kind = "create" },
    ["lane-2"] = { kind = "unmapped" },
...
assert(result.items[1].context.position_seconds == 3, "Reference-relative position applied")
assert(result.items[1].context.source_project_id == "source-1", "Source binding context")
assert(result.items[1].context.lane_id == "lane-1", "Delivery Lane ownership context")
local subscriptions = json.decode(values.delivery_subscriptions)
assert(subscriptions[1].deliveryId == "set-1", "subscription persisted")
...
assert(subscriptions[1].lanes[2].unmapped, "unmapped Lane persisted")

local duplicate, duplicate_error = delivery_import.review(adapter, fs, pointer_path)
assert(not duplicate and duplicate_error:find("already subscribed", 1, true), "duplicate subscription blocked")

local removed = assert(delivery_import.remove_subscription(adapter, "source-1"))
assert(removed.source_project_id == "source-1", "subscription removal result")
assert(#json.decode(values.delivery_subscriptions) == 0, "subscription removed without project content mutation")
...
local failed, failed_error = delivery_import.apply(review, adapter, {
  allow_reference_revision_mismatch = true,
  mappings = {
    ["lane-1"] = { kind = "create" },
    ["lane-2"] = { kind = "unmapped" },
...
assert(events[#events] == "cancel:Import ReaProjectLink Delivery", "failed import rolls back its undo block")

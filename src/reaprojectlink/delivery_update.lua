...
function M.review(adapter, fs, source_project_id, target_revision)
  ...
  -- target the Master Project is aligned to.
  ...
  if master_reference_id ~= snapshot.reference.referenceId then
  ...
  -- Several Linked Items may legitimately carry one Clip, so the review numbers
  ...
  for _, instance in ipairs(instances) do
    ...
    local baseline_revision = instance.handled_delivery_revision
    local baseline = baseline_cache[baseline_revision]
    if not baseline then
      local baseline_path = fs.join(
        package_root,
        string.format("history/delivery-%04d.json", baseline_revision)
      )
    ...
    row.plan = delivery_update_plan.build({
  ...
  for _, lane in ipairs(snapshot.lanes or {}) do
    ...
    -- Deleting the bound Master Track must not strand the Lane; it becomes
    ...
    if not binding or binding.unmapped or orphaned then
      ...
      for _, clip in ipairs(lane.clips or {}) do
        -- A Clip that already has a Linked Item stays where the Master user put it,
    ...
    elseif binding.trackGuid then
      -- Moving an Item elsewhere never breaks its Link, but the Lane still
      ...
      for _, clip in ipairs(lane.clips or {}) do
        -- A Clip belongs in the Master Project unless a Linked Item already
        -- carries it or the Master user declined it.
...
function M.apply(review, adapter, options)
  ...
  for _, addition in ipairs(review.additions) do
    if addition_decisions[addition.clip.clipId] ~= "import" and
      ...
      return nil, "Every Pending Clip requires Import Clip or Decline Clip."
    end
  end
  ...
  if adapter.valid_item then
    for _, row in ipairs(review.instances) do
      ...
      if adapter.delivery_instance_state and
          json.encode(adapter.delivery_instance_state(row.item_ref)) ~= json.encode(row.local_state) then
  ...
  for _, addition in ipairs(review.additions) do
    if addition_decisions[addition.clip.clipId] == "import" then
      ...
      if not track then
        cancel_undo(adapter, "Apply ReaProjectLink Delivery Update")
        return nil, "Target Track Missing."
      end
  ...
  for _, lane_review in ipairs(review.unmapped_lanes) do
    ...
    if mapping.kind == "unmapped" then
      ...
      if mapping.kind == "create" then
        track = adapter.create_master_track(lane_review.display_name, mapping.parent_track_ref)
  ...
  for _, addition in ipairs(review.additions) do
    declined[addition.clip.clipId] = addition_decisions[addition.clip.clipId] == "decline" or nil
  end
  local declined_list = {}
  ...
  adapter.end_undo("Apply ReaProjectLink Delivery Update")
  return result
end
...
function M.undecline(adapter, source_project_id, clip_id)
  ...
  if not ok or type(subscriptions) ~= "table" then
    return nil, "Stored Delivery subscriptions are invalid."
  end
...
function M.detach(adapter, item)
  ...
  if not detached then
    cancel_undo(adapter, "Detach ReaProjectLink Linked Item")
    return nil, detach_error
  end
  adapter.mark_project_dirty()
  adapter.end_undo("Detach ReaProjectLink Linked Item")

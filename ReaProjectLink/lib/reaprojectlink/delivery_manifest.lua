...
function M.build(input)
  local revision = required(input, "delivery_revision")
  ...
  local snapshot = {
    ...
    deliveryId = required(input, "delivery_id"),
    ...
    reference = {
      ...
      reviewedRevision = required(input, "reviewed_reference_revision"),
    },
  ...
  local pointer = {
    ...
    latestDeliveryRevision = revision,
    manifest = string.format("history/delivery-%04d.json", revision),
  }
